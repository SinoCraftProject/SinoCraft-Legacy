...
public class GuiChineseBrush extends ContainerScreen<ContainerChineseBrush> {
    ...
    protected void init() {
        super.init();
        buttonUp = new Button(guiLeft + 16, guiTop + 112,11,7,"null",(button)->{this.container.incColor();}) {
            @Override
        ...
        buttonDown = new Button(guiLeft + 16, guiTop + 166,11,7,"null",(button)->{this.container.decColor();}) {
            @Override
            public void render(int mouseX, int mouseY, float partialTicks) {
                this.isHovered = mouseX >= this.x && mouseY >= this.y && mouseX < this.x + this.width && mouseY < this.y + this.height;
                Minecraft.getInstance().getTextureManager().bindTexture(GUI);
