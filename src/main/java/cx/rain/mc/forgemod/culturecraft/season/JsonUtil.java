...
public class JsonUtil
{
    ...
    public static <T> T getOrCreateConfigFile(File configDir, String configName, T defaults, Type type)
    {
        ...
        {
            CultureCraft.Log.error("Error parsing config from json: " + configFile.toString(), e);
        }
    ...
    protected static boolean writeFile(File outputFile, Object obj)
    {
        ...
        {
            CultureCraft.Log.error("Error writing config file " + outputFile.getAbsolutePath() + ": " + e.getMessage());
            return false;
        }
    }
}